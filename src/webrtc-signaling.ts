  ...
  try {
    ...
    cookieSessionMiddleware(req as any, {} as any, async () => {
      try {
        ...
        const { deviceId, token, userId, secretToken } = await authenticateClientRequest(req as any);
        if (!deviceId) {
          return socket.destroy();
        }
...
async function authenticateClientRequest(req: Request & { session: any }) {
  ...
  try {
    ...
    const device = await prisma.device.findUnique({
      where: { id: deviceId, user: { id: BigInt(sub as string) } },
    ...
    return { deviceId, token, userId: sub as string, secretToken: device.secretToken ?? "" };
  } catch (error) {
